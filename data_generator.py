...
def readcsv(filename, image_PATH, data_source, train_or_test):
    ...
    elif data_source == 'visda-2017':
        # all_class = [, ]

        train_class = ['aeroplane', 'bus', 'horse', 'knife', 'person', 'skateboard']

        test_class = ['truck', 'bicycle', 'car', 'motorcycle', 'plant', 'train']

        val_class = ['aeroplane', 'bus', 'horse', 'knife', 'person', 'skateboard']
        sets = {'train': train_class, 'val': val_class, 'test': test_class}
        return sets[train_or_test]
...
def sample_task(data_model, raw_class_num, label_text, model, query_num_per_class_per_model=1, class_num=5,
    ...
    task = {'x': {'support': [], 'query': []},
            ...
            'label_text':[]}
    for c in classes:
        task['label_text'].append(label_text[c])
...
def split_dataset(model, split_txt, mode, data_source='mini-Imagenet', data_PATH=None):
    ...
    elif data_source == 'officehome' or 'visda' or 'tiered-Imagenet' or 'visda-2017':
    ...
    for k, m in enumerate(model):
        ...
        total_group[m] = group
    return total_group, class_num, fliename_label



class DataGenerator(object):
    def __init__(self, mode, datasource, imagesize, episode, way_num, support_num, query_num, data_dir, split_PATH, preprocess, source_domain=None, target_domain=None, random_target=False):
        ...
        self.split_txt, self.model, self.train_test = self.construct(datasource, split_PATH, preprocess)
        self.data_model, self.raw_class_num, self.label_text = split_dataset(self.model, self.split_txt, self.mode, self.datasource,
                                                            self.raw_path)
        self.random_target = random_target
        self.all_tasks = self.generator_tasks()
        self.now_epi = None

    def construct(self, datasource, split_PATH, preprocess):
        ...
        self.data_parser = Parsers(datasource, self.raw_path, self.img_size, self.mode, preprocess)
    ...
    def generator_tasks(self,):
        ...
        for _ in (range(tasknum)):
            task = sample_task(self.data_model, self.raw_class_num, self.label_text, model, query_num_per_class_per_model=self.query_num,
    ...
    def __getitem__(self, index):
        ...
        query_images = torch.cat((torch.stack(task[0]['query'][0], 0), torch.stack(task[0]['query'][1], 0)),0 )

        query_targets = task[1]['query']
        query_modal = task[2]['query']

        support_images = torch.stack([torch.stack(task[0]['support'][0], 0), torch.stack(task[0]['support'][1],0)], 0).reshape(2, self.way_num, self.support_num, 3, 224, 224)
...
class Parsers:
    def __init__(self, datasource, data_PATH, img_size, mode, preprocess):
    ...
    def get_data(self, key, dict_set):
        ...
        for m in range(len(imgs)):
            ...
            img_tensorset.append(imgtensor)
