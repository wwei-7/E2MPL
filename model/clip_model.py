...
class ClipAdaptor(BaseModelFSL):
    ...
    def get_grads(self):
        grads = []
        pp = self.fc.parameters()
        for p in self.fc.parameters(): # Parameter:(256, 512)  (256,)  (256, 128)  (128,)
    ...
    def proj_grad(self, domain_grads):
        new_grads = []
        start = 0
        for k, p in enumerate(self.fc.parameters()):
            dims = p.shape
            end = start + dims.numel()
            classigy_grads = domain_grads[0][start:end]
            da_grads = domain_grads[1][start:end]
            aa = classigy_grads*da_grads
            bb = torch.sum(da_grads*da_grads)+1e-12
            inner_product = torch.sum(classigy_grads*da_grads)
            proj_direction = inner_product / (torch.sum(
                da_grads*da_grads)+1e-12)
            cc = torch.zeros_like(proj_direction)
            dd = torch.min(proj_direction, torch.zeros_like(proj_direction))
